...
Detect malicious bot invitations or automated attacks.
Detect behavior that violates the server's rules.
Assist moderators in enforcing server rules.
Automatically warn, mute, kick, or ban users when configured by the server administrators.
...
Mass mentions.
Raid behavior.
Inviting or using malicious bots.
Other behaviors configured by the server administrators.
...
When necessary to investigate abuse or security incidents.
When viewed by authorized moderators or administrators of the Discord server.
Data Security
...
Informing their members that the Bot is being used.
Ensuring the Bot's moderation features comply with applicable laws and Discord's Terms of Service.
Configuring moderation settings appropriately for their community.
